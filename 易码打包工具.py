...
def 显示帮助():
    ...
    print("将会在当前目录下生成一个可以直接运行、并带有专属图标的 .exe 文件！")
...
def 编译并打包(源码路径, 图标路径=None, 隐藏黑框=False, 进度打字机=print):
    ...
    if not 如果你有图标:
        默认图标路径 = os.path.join(当前目录, "logo.ico")
...
"""
    ...
    try:
        ...
        subprocess.run("pip install pyinstaller", check=True, shell=True)
        
    进度打字机(f"🚀 开始将打包成了不起的 EXE 外挂...")
    yima核心库路径 = os.path.join(当前目录, 'yima')
    分隔符 = ';' if os.name == 'nt' else ':'
    
    # 自动把源码同目录下的所有 .ym 文件都打包进去，支持多文件项目
    源码所在目录 = os.path.dirname(绝对源码路径)
    额外数据参数 = ""
    for file in os.listdir(源码所在目录):
        if file.endswith(".ym"):
            完整路径 = os.path.join(源码所在目录, file)
            额外数据参数 += f' --add-data "{完整路径}{分隔符}."'
            
    控制台参数 = "--windowed" if 隐藏黑框 else "--console"
    命令 = f'pyinstaller --noconfirm --onefile{图标参数} {控制台参数} --hidden-import=turtle --hidden-import=tkinter --hidden-import=tkinter.messagebox{图标数据参数}{额外数据参数} --add-data "{yima核心库路径}{分隔符}yima"  "{启动器脚本}"'
    ...
    process = subprocess.Popen(命令, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding=sys_encoding, errors='replace')
